...
def CCA_Analysis(orig_image, predict_image, erode_iteration, open_iteration):
    ...
    kernel_1 = (np.ones((5, 5), dtype=np.float32))
    ...
    image = cv2.morphologyEx(image, cv2.MORPH_OPEN, kernel_1, iterations=open_iteration)
    image = cv2.filter2D(image, -1, kernel_sharpening)
    image = cv2.erode(image, kernel_1, iterations=erode_iteration)
    ...
    for label in a:
        ...
        cv2.line(image2, (int(tltrX), int(tltrY)),
                 (int(blbrX), int(blbrY)), color, 2)
        cv2.line(image2, (int(tlblX), int(tlblY)),
                 (int(trbrX), int(trbrY)), color, 2)
        ...
        cv2.putText(image2, "{:.1f}pixel".format(dimA), (int(
            tltrX - 15), int(tltrY - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.65, color, 2)
        cv2.putText(image2, "{:.1f}pixel".format(dimB), (int(
            trbrX + 10), int(trbrY)), cv2.FONT_HERSHEY_SIMPLEX, 0.65, color, 2)
        cv2.putText(image2, "{:.1f}".format(label), (int(
            tltrX - 35), int(tltrY - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.65, color, 2)
